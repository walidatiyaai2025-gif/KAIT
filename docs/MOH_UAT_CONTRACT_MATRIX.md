...
| --- | --- | --- | --- | --- | --- | --- |
| MOH Certificate Information | `https://moh-uat.api-non-prod.cait.gov.kw/prmapi/moj/v1` | `POST /certificate` | `POST /authenticate` | JSON `{ username, password }` | `data` | Bearer token only; no additional operation-level authentication is inferred |
...
Each GSIP service receives an independent UAT `TokenEndpoint` AuthProfile and exact Service + Environment binding. Profiles are seeded disabled with no secret material. The administrator must store only the protected credentials proved by the operation-level provider contract and explicitly enable the profile/configuration before execution.

For MOH Certificate Information, the supplied evidence proves `POST /authenticate` with JSON `username`/`password`, token extraction from `data`, and Bearer use for the target request. The supplied security-scheme metadata does not establish operation-level `x-api-key` use on either `/authenticate` or `/certificate`, so GSIP does not configure, seed, or transmit an API key for this service unless later direct provider evidence proves that requirement.

...
| P06 | OPEN | Secret vault, isolated authentication profiles/SecretRefs, explicit Shared AuthProfile support, rotation/redaction, cross-service-safe token cache keys |
...
P06 is the next legal phase only after this P05 closure-reconciliation change is normally integrated to `main` and the resulting exact-main closed-phase regressions remain green.

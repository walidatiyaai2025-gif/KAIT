...
require('SetProperty(item => item.NonSecretHeadersJson, "{}")' in controller, "TokenEndpoint metadata is not removed for API-key-only UAT execution.")
...
require(
    "BeginTransactionAsync(IsolationLevel.Serializable" in controller
    and "transitionTransaction.RollbackAsync(CancellationToken.None)" in controller,
    "Legacy token-slot retirement is not protected by a serializable rollback-capable transaction.",
)
require(
    "secretVault.RevokeAsync(" in controller
    and ".ExecuteDeleteAsync(cancellationToken)" in controller,
    "Legacy secret material and slot metadata are not retired together.",
)
require(
    "profile.Bindings.Count != 1" in controller
    and "!binding.IsShared" in controller
    and "HasExclusiveOwnerBinding(profile, service.Id)" in controller,
    "Exclusive owner-binding protection is missing.",
)
require(
    '"BINDING_MISMATCH_DISABLED"' in controller
    and "DisableConfigurationOnlyAsync" in controller,
    "Disable action does not fail closed on a forged or shared AuthProfile binding.",
)
require(
    "profile.Secrets.Count != 1" in controller
    and "profile.Secrets.Count == 1" in controller,
    "API-key-only convergence does not require exactly one secret slot.",
)
require(
    "profile.AuthType != AuthProfileType.TokenEndpoint" in controller,
    "Legacy-slot retirement is not constrained to the known TokenEndpoint migration shape.",
)

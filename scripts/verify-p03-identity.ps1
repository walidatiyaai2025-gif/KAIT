...
try {
    ...
    try {
        ...
        if (-not $location.StartsWith('/login', [System.StringComparison]::OrdinalIgnoreCase)) { throw "Unauthenticated dashboard did not redirect to login. Location: $location" }

...
static void CheckPhaseBoundaries(string root, List<string> failures)
{
    ...
    var p03OrLater = Enumerable.Range(3, 15).Any(number => currentPhase.Contains($"P{number:00} —", StringComparison.Ordinal));
    ...
    if (p03OrLater)
    {
        Expect(login.Contains("data-auth-state=\"active\"", StringComparison.Ordinal), "P03+ must deliberately replace the P01 shell-only login boundary with active authentication.", failures);
        Expect(login.Contains("method=\"post\"", StringComparison.Ordinal), "P03+ login must submit through a protected POST flow.", failures);
        Expect(!login.Contains("type=\"button\" disabled", StringComparison.Ordinal), "P03+ login must not retain the obsolete disabled P01 sign-in control.", failures);
    }
    else
    {
        Expect(login.Contains("data-auth-state=\"shell-only\"", StringComparison.Ordinal), "Before P03, login must retain the closed-P01 shell boundary.", failures);
        Expect(login.Contains("type=\"button\" disabled", StringComparison.Ordinal), "Before P03, the P01 login shell must remain non-submitting.", failures);
    }
